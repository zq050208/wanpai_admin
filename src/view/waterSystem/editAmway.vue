  ...
  <Card>
    ...
    <Row type="flex" align="middle" justify="center">
      ...
      <Card>
        ...
        <div class="container" v-html="contentData.content"></div>
...
<script>
import { getAssessInfo, getUserData, getCommentGroupData, getUserGroupData, autoGameCommentContent, addGameComment, likeGameAssess } from '@/api/data'
...
export default {
  ...
  methods: {
    ...
    /**
       ...
       */
    getAssessInfo () {
      getAssessInfo({ assess_id: this.commentData.content_id }).then(res => {
        if (res.data.code === 200) {
          this.contentData = res.data.data
          this.commentData.game_id = this.handCommentData.game_id = res.data.data.game_id
          this.commentData.assess_id = this.handCommentData.assess_id = res.data.data.id
    ...
    addCommentClick () {
      if (this.tabIndex === 0) {
        ...
        this.$refs.commentData.validate((valid) => {
          if (valid) {
            this.loading = true
            autoGameCommentContent(this.commentData).then(res => {
              this.loading = false
              if (res.data.code === 200) {
        ...
        this.$refs.handCommentData.validate((valid) => {
          if (valid) {
            this.loading = true
            addGameComment(this.handCommentData).then(res => {
              this.loading = false
              if (res.data.code === 200) {
                this.tabIndex = 0
                this.$Message.success(res.data.msg)
    ...
    likeGameAssess () {
      let data = {
        assess_id: this.commentData.content_id,
        user_num: this.addVal,
        game_id: this.commentData.game_id,
        parent_user_id: this.commentData.parent_user_id
      }
      console.log(data)
      likeGameAssess(data).then(res => {
        console.log(res)
        if (res.data.code === 200) {
          this.$Message.success(res.data.msg)
          this.getAssessInfo()
