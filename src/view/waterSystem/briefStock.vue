<template>
  <div>
    <Row style="background:#eee;padding:20px">
      <Col span="11">
        <Card>
          <cardHeader title="未使用简介" leftContent="删除简介" rightContent="导入简介" @on-left-click="leftBtnClick" @on-right-click="rightBtnClick"/>
          <Tabs :value="tabs_value1" type="card" style="margin-top: 16px" @on-click="handleTabs">
            <TabPane label="男性简介" name="name1">
      ...
      <Col span="11" offset="2">
        <Card>
          <cardHeader title="已使用简介" rightContent="导出简介" :loading="loading" @on-right-click="exportNickName"/>
          <Tabs :value="tabs_value2" type="card" style="margin-top: 16px" @on-click="handleTabs">
            <TabPane label="男性简介" name="name3">
...
export default {
  ...
  mounted () {
    this.getSignatureList(0)
    this.getSignatureList(2)
  },
  methods: {
    /**
     * 获取虚拟简介列表
     * @param type 0：男性未使用简介  1：女性未使用简介  2：男性已使用简介  3：女性未使用简介
     */
    getSignatureList (type) {
      let data = {}
      switch (type) {
        case 0:
          data = {
      ...
      getSignatureList(data).then(res => {
    ...
    delNickName () {
      if (this.selection.length) {
        ...
        delSignature({ signature_id: ids, is_use: 1 }).then(res => {
          if (res.data.code === 200) {
            ...
            if (this.tabs_value1 === 'name1') {
              ...
              this.selection = []
              this.getSignatureList(1)
            }
          }
        })
      } else {
        this.$Message.error({ content: '请选择需要删除的简介' })
      }
    },
    /**
     * card1 左边按钮
     ...
     **/
    rightBtnClick () {
      let action = server.BASE_URL + 'VirtualSignature/importSignatureExcel'
      let token = getToken()
      this.$Modal.info({
        okText: '关闭',
        render: (h) => {
