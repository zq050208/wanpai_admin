  ...
  <div>
    ...
    >
      <Form
        ...
        inline
      >
        <FormItem prop="site_name" label="外站站名：" style=" width:80%">
          <Input
            type="text"
        ...
        <FormItem label="外站昵称：" prop="external_name" style=" width:80%">
          <Input
            type="text"
            v-model="formData.external_name"
            maxlength="18"
            clearable
          ></Input>
        </FormItem>
        ...
        <FormItem prop="image_one" label="个人认证截图:" style=" width:80%">
          <div
        ...
        <FormItem prop="image_two" label="达人认证截图:" style=" width:80%">
          <div
            class="demo-upload-list"
    ...
    <Modal v-model="bigPicvisible">
      ...
      <span slot="footer"
        ><Button type="primary" @click="bigPicvisible = false"
          >返回</Button
        ></span
      >
    </Modal>
...
<script>
import {
  getMasterApplyOutStationList,
  deleteMasterApply,
  getsaveOutStationView,
  GameMetricsAdminCheckCode,
  editorMasterApplyOut,
  getQiniuToken,
  approveMasterApply,
  refuseMasterApply
} from "@/api/data";
...
export default {
  ...
  data() {
    return {
        ...
        },
        {
          ...
          render: (h, params) => {
            if (params.row.image_one) {
              return h("div", [
                h("img", {
                  attrs: { src: params.row.image_one },
                  style: { width: "100px", height: "100px", margin: "auto" },
                  on: {
                    click: () => {
                      this.showPics(params.row.image_one, "image_one");
                    }
                  }
                })
              ]);
      ...
      rules: {
        site_name: [{ required: true, message: "请输入外站站名" }],
        external_name: [{ required: true, message: "请输入外站昵称" }],
        profile: [{ required: true, message: "请输入达人简介" }],
        image_one: [{ required: true, message: "请选择上传个人认证截图" }],
        image_two: [{ required: true, message: "请选择上传达人认证截图" }]
      },
      searchData: {
        keyword: "",
        date: "",
        ...
        actions: [
          {
            ...
            handle: () => {
              ...
              this.getMasterApplyOutStationList();
            }
          }
        ],
        components: [
  ...
  methods: {
    ...
    getMasterApplyOutStationList() {
      ...
      getMasterApplyOutStationList(data).then(res => {
    ...
    getMasterApplyDetail(id) {
      ...
      getsaveOutStationView(dataId).then(res => {
        if (res.data.code === 200) {
    ...
    editorMasterApplyOut() {
      ...
      editorMasterApplyOut(data).then(res => {
        if (res.data.code === 200) {
