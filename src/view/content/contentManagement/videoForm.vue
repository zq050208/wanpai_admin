<template>
  <Form ref="editForm" :model="formData" :rules="rules" :label-width="100" label-position="right">
    ...
    <FormItem label="上传视频：" prop="video">
      <Upload
        disabled 
        accept="video/mp4"
    ...
    <FormItem label="标签：" required>
      <div class="tag">
        ...
        >+添加标签（{{ selectTags.length }}/10）</a>
      </div>
    </FormItem>

    <FormItem>
      <Button type="primary" size="large" :loading="loading" @click="handleSubmit">发布</Button>
...
export default {
  ...
  methods: {
    ...
    handleVideoSuccess(response) {
      const videoUrl = baseImgUrl + response.key
      this.$emit('videoChange', videoUrl)
    },
    handleVideoRemove() {
