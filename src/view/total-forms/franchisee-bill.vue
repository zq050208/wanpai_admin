...
<script>
import { getFranchiseeBill, settleBill } from '@/api/data'
import { getNowFormatDate } from '@/view/forms/time'
export default {
  name: 'franchisee-bill',
  inject: ['reload'],
  data () {
    return {
      modal1: false,
      ...
      columns: [
        ...
        },
        {
          ...
          key: 'is_bill',
          render: (h, params) => {
            return h('div', {}, params.row.bill_status === 1 ? '未结算' : '已结算')
          }
        },
        {
          title: '操作',
          render: (h, params) => {
            return h('div', [
              h('Button', {
                props: { type: 'primary', size: 'small', disabled: params.row.bill_status !== 1 },
                on: { click: () => {
                  this.$Modal.confirm({
                    title: '提示',
                    content: '是否结算' + params.row.channel_name + params.row.bill_date + '的账单',
                    onOk: () => {
                      settleBill({ bill_id: params.row.id }).then(res => {
  ...
  methods: {
    getIndexMonthForm () {
      ...
      getFranchiseeBill(data).then(res => {
